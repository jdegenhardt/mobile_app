...
function essen_makeRequest(id, attribute) {
	essen_http_request = new XMLHttpRequest();
	
	var url = "data/fb2json_essen.php?id=" + id + "&attribute=" + attribute;
    essen_http_request.open("GET", url, true);
    essen_http_request.onreadystatechange = function() {
		var done = 4, ok = 200;
		if (essen_http_request.readyState == done
				&& essen_http_request.status == ok) {
			json = JSON.parse(essen_http_request.responseText);
			essen_handleRequest();
		}
	};
    essen_http_request.send(null);
}
